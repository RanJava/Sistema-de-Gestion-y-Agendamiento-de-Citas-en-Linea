...
1. **Solapamiento y dobles reservas** debido a la falta de un control transaccional centralizado.
2. **Pérdida de productividad e interrupciones constantes:** los barberos y estilistas deben pausar sus servicios para responder llamadas o mensajes.
3. **Ausencia de visibilidad en tiempo real:** los clientes desconocen la disponibilidad real del profesional de su preferencia, aumentando los tiempos de espera presenciales o provocando inasistencias sin previo aviso (*no-shows*).
4. **Falta de métricas y control operativo:** los administradores carecen de trazabilidad sobre el estado de las citas (pendientes, atendidas, canceladas) y el volumen diario de atención.
...
- **Capa de Aplicación / Negocio (Backend):** API RESTful desarrollada en Node.js con Express y TypeScript, encargada de la validación de reglas de negocio, sincronización de concurrencia y orquestación transaccional.
- **Capa de Persistencia (Base de Datos):** Motor Relacional PostgreSQL / Cloud SQL asegurando integridad referencial ACID estricta para evitar dobles reservas.
...
| **Backend / API** | Node.js, Express 4.x / 5.x, TypeScript (`tsx`, `esbuild`) | Alto rendimiento para manejo concurrente de consultas I/O no bloqueantes y validaciones de disponibilidad. |
| **Base de Datos** | PostgreSQL 15+ / Cloud SQL | Soporte nativo de transacciones ACID, tipos `TIME`, `DATE`, `TIMESTAMP`, índices únicos y claves foráneas con políticas de borrado (`CASCADE`/`RESTRICT`). |
| **Seguridad y Criptografía** | `bcrypt` / `argon2` para hashing de contraseñas, JSON Web Tokens (JWT) / Cookies HttpOnly para sesiones seguras. | Cumplimiento del principio de no almacenar credenciales en texto plano (Ley 164 / D.S. 1793). |
| **Control de Versiones & CI/CD** | Git, GitHub, Vite Build System | Integración continua, trazabilidad del código y despliegue automatizado. |
...
#### 1. Tabla: `CLIENTE`
Almacena la información de los usuarios finales que reservan citas en el sistema.
...
#### 2. Tabla: `BARBERO`
Almacena a los profesionales estilistas/barberos que prestan servicios en el local.
| Atributo | Tipo de Dato | Nulo | Clave | Restricciones / Reglas | Descripción |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
#### 3. Tabla: `HORARIO_DISPONIBILIDAD`
Define la plantilla de horarios semanales recurrentes asignados a cada barbero.
| Atributo | Tipo de Dato | Nulo | Clave | Restricciones / Reglas | Descripción |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
#### 4. Tabla: `TURNO`
Representa los slots o intervalos específicos generados por fecha para cada barbero.
| Atributo | Tipo de Dato | Nulo | Clave | Restricciones / Reglas | Descripción |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
#### 5. Tabla: `SERVICIO`
Catálogo de cortes y tratamientos ofrecidos en la barbería.
| Atributo | Tipo de Dato | Nulo | Clave | Restricciones / Reglas | Descripción |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
#### 6. Tabla: `CITA`
Registro de la reserva concretada entre cliente, servicio y turno.
| Atributo | Tipo de Dato | Nulo | Clave | Restricciones / Reglas | Descripción |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
El sistema **BarberLosPeluchitos** se rige estrictamente bajo el marco jurídico vigente del Estado Plurinacional de Bolivia:

### 4.1 Ley N° 164 (Ley General de Telecomunicaciones, Tecnologías de Información y Comunicación)
* **Artículo 2 (Principios):** Principios de confidencialidad, integridad y seguridad en el uso de medios y servicios digitales.
* **Artículo 6 (Protección de Datos Personales):** El sistema garantiza que los datos personales recabados (nombre, teléfono, correo) serán utilizados exclusivamente para la gestión operativa de citas y no serán cedidos ni comercializados a terceros sin consentimiento expreso.
* **Artículo 79 y ss. (Comercio Electrónico y Mensajes de Datos):** Reconocimiento de la validez legal y probatoria de las transacciones electrónicas, confirmaciones automáticas digitales y cancelaciones cursadas a través de la WebApp.

### 4.2 Decreto Supremo N° 1793 (Reglamento a la Ley N° 164 para el Sector TIC)
* **Seguridad de la Información y Gestión Criptográfica:** Obligatoriedad de implementar mecanismos técnicos de protección sobre credenciales y accesos. Se prohíbe el almacenamiento de contraseñas en texto plano, obligando el uso de algoritmos criptográficos robustos de hash con salting (`contrasena_hash`).
...
* **Artículo 12 y 13 (Transparencia en Precios y Ofertas):** Garantía de preservación de tarifa (*snapshot de precio*) acordada al momento de la reserva en caso de variaciones tarifarias posteriores en el catálogo de servicios.
* **Artículo 35 (Condiciones de Cancelación):** El cliente tiene derecho a desistir o cancelar su cita con anticipación mediante la plataforma sin imposición de cobros arbitrarios o penalizaciones abusivas no pactadas.
...
* **Criterios de Aceptación:**
  1. **Escenario 1 (Registro exitoso):**
     * **Dado que** el cliente se encuentra en la pantalla de registro,
     * **Cuando** completa su nombre, teléfono y correo con datos válidos y confirma,
     * **Entonces** el sistema encripta la contraseña, persiste la cuenta en `CLIENTE` y redirige al login o al inicio.
  2. **Escenario 2 (Correo duplicado):**
     * **Dado que** se intenta registrar una cuenta con un correo ya existente en la base de datos,
     * **Cuando** se envía el formulario,
     * **Entonces** el sistema bloquea el registro y muestra un error indicando que el correo ya está en uso.
  3. **Escenario 3 (Campos obligatorios vacíos):**
     * **Dado que** el usuario omite ingresar el nombre, teléfono o correo,
     * **Cuando** intenta confirmar el registro,
     * **Entonces** el sistema resalta los campos faltantes y no envía la petición.
  4. **Escenario 4 (Validación de formato de correo):**
     * **Dado que** se ingresa un correo con formato sintáctico inválido,
     * **Cuando** se valida el formulario,
     * **Entonces** el sistema muestra un mensaje de validación de formato.
...
* **Prioridad:** Alta | **Sprint:** Semana 1
* **Criterios de Aceptación:**
  1. **Escenario 1 (Alta de personal y horario):**
     * **Dado que** el administrador ingresa el nombre, teléfono y franjas horarias semanales de un barbero,
     * **Cuando** confirma el formulario,
     * **Entonces** el sistema almacena el registro en `BARBERO` y `HORARIO_DISPONIBILIDAD`, listándolo en el catálogo del staff.
  2. **Escenario 2 (Actualización de disponibilidad en tiempo real):**
     * **Dado que** un barbero ya cuenta con horarios registrados,
     * **Cuando** el administrador modifica su jornada,
     * **Entonces** el sistema actualiza de forma inmediata la disponibilidad en el módulo público de reservas.
  3. **Escenario 3 (Validación de rango horario inconsistente):**
     * **Dado que** se intenta registrar una franja donde la hora de fin es igual o anterior a la hora de inicio,
     * **Cuando** se envía el formulario,
     * **Entonces** el sistema rechaza la operación y notifica el error de rango inválido.
  4. **Escenario 4 (Barbero sin disponibilidad):**
     * **Dado que** un barbero no tiene horarios cargados en el sistema,
     * **Cuando** un cliente busca disponibilidad en la WebApp,
     * **Entonces** dicho barbero no figura como opción disponible para agendamiento.
...
* **Prioridad:** Alta | **Sprint:** Semana 1
* **Criterios de Aceptación:**
  1. **Escenario 1 (Visualización de turnos libres):**
     * **Dado que** el cliente selecciona un barbero y una fecha,
     * **Cuando** consulta la agenda,
     * **Entonces** el sistema muestra únicamente los slots con estado `Disponible` dentro de la jornada de trabajo del profesional.
  2. **Escenario 2 (Fecha sin turnos libres):**
     * **Dado que** no existen turnos disponibles para la fecha consultada,
     * **Cuando** se procesa la consulta,
     * **Entonces** el sistema emite el mensaje *"Sin turnos disponibles para esta fecha"* y ofrece el botón para saltar al día siguiente.
  3. **Escenario 3 (Bloqueo por concurrencia simultánea):**
     * **Dado que** un turno es reservado por otro usuario mientras el cliente observaba la pantalla,
     * **Cuando** este intenta seleccionarlo,
     * **Entonces** el sistema refresca la vista, bloquea el slot e informa que el horario acaba de ser tomado.
  4. **Escenario 4 (Filtrado de horarios pasados para el día actual):**
     * **Dado que** se consulta la disponibilidad para el día de hoy,
     * **Cuando** se cargan los horarios,
     * **Entonces** el sistema filtra y omite automáticamente los turnos cuya hora de inicio ya transcurrió.
...
* **Prioridad:** Alta | **Sprint:** Semana 1
* **Criterios de Aceptación:**
  1. **Escenario 1 (Habilitación de confirmación):**
     * **Dado que** el usuario ha seleccionado satisfactoriamente el servicio, el barbero y un horario disponible,
     * **Cuando** los 3 parámetros son válidos,
     * **Entonces** el sistema activa el botón de confirmación de reserva.
  2. **Escenario 2 (Control de usuario no autenticado):**
     * **Dado que** un usuario navega como invitado e intenta confirmar una reserva,
     * **Cuando** presiona reservar,
     * **Entonces** el sistema intercepta el flujo exigiéndole iniciar sesión o crear una cuenta.
  3. **Escenario 3 (Manejo de condiciones de carrera):**
     * **Dado que** dos clientes confirman el mismo turno simultáneamente,
     * **Cuando** las peticiones impactan en el backend,
     * **Entonces** el sistema adjudica el turno al primer requerimiento creando la cita en estado `Pendiente` y notifica al segundo usuario para que elija otro turno.
  4. **Escenario 4 (Cálculo de snapshot de precio y duración):**
     * **Dado que** la reserva es confirmada exitosamente,
     * **Cuando** se registra la fila en `CITA`,
     * **Entonces** el sistema guarda la duración y el precio vigentes del servicio y cambia el estado del turno a `Reservado`.
...
* **Criterios de Aceptación:**
  1. **Dado que** la cita ha sido registrada satisfactoriamente en la base de datos,
  2. **Cuando** finaliza la transacción,
  3. **Entonces** la interfaz muestra una vista modal/pantalla de confirmación con el código de cita, nombre del barbero, servicio, fecha, hora y monto a cancelar en caja.
...
* **Criterios de Aceptación:**
  1. **Dado que** el cliente visualiza su cita activa en estado `Pendiente`,
  2. **Cuando** presiona *"Cancelar Cita"* y confirma el cuadro de diálogo,
  3. **Entonces** el sistema actualiza el estado de la cita a `Cancelada` y libera el turno asociado cambiando su estado a `Disponible` de inmediato.
...
* **Prioridad:** Alta | **Sprint:** Semana 2
* **Criterios de Aceptación:**
  1. **Dado que** el administrador ingresa al panel de control,
  2. **Cuando** selecciona la fecha de consulta (por defecto la fecha actual),
  3. **Entonces** el sistema lista todas las citas del día ordenadas cronológicamente, indicando cliente, teléfono, barbero asignado, servicio, hora y estado actual.
...
* **Prioridad:** Alta | **Sprint:** Semana 2
* **Criterios de Aceptación:**
  1. **Dado que** una cita se encuentra en estado `Pendiente` en el panel administrativo,
  2. **Cuando** el administrador marca la cita como `Atendida` tras finalizar el corte,
  3. **Entonces** el sistema guarda el nuevo estado y actualiza las métricas del día.
  4. **Cuando** el administrador marca la cita como `Cancelada` por inasistencia,
  5. **Entonces** el sistema registra la cancelación y permite liberar el slot si aplica.
...
### 7.1 HU-01: Registro de Cuenta de Cliente
```plantuml
@startuml HU01_casos_uso
left to right direction
actor Cliente
rectangle "Sistema BarberLosPeluchitos" {
 usecase "Registrar cuenta" as UC1
 usecase "Mostrar error: correo ya registrado" as UC2
 usecase "Mostrar error: campo obligatorio vacio" as UC3
 usecase "Mostrar error: formato de correo invalido" as UC4
}
Cliente --> UC1
UC2 .> UC1 : <<extend>>
UC3 .> UC1 : <<extend>>
...
```plantuml
@startuml HU01_secuencia
actor Cliente
participant "Interfaz de Registro" as UI
participant "Controlador de Cuenta" as CTRL
database "Base de Datos" as DB

Cliente -> UI : Completa nombre, telefono, correo
UI -> CTRL : enviarRegistro(nombre, telefono, correo)
alt Campos vacios o formato invalido
 CTRL -> UI : error de validacion (campo/formato)
 UI --> Cliente : Muestra mensaje de error
else Datos validos
 CTRL -> DB : buscarClientePorCorreo(correo)
 DB --> CTRL : resultado
 alt Correo ya registrado
 CTRL -> UI : error "correo ya en uso"
 UI --> Cliente : Muestra mensaje de error
 else Correo disponible
 CTRL -> DB : guardarCliente(nombre, telefono, correo)
 DB --> CTRL : confirmacion de guardado
 CTRL -> UI : registro exitoso
 UI --> Cliente : Redirige a login/home
 end
end
@enduml
```

### 7.2 HU-02: Registro de Barberos y Horarios
```plantuml
@startuml HU02_casos_uso
left to right direction
actor Administrador
rectangle "Sistema BarberLosPeluchitos" {
 usecase "Registrar barbero" as UC1
 usecase "Cargar horarios de disponibilidad" as UC2
 usecase "Editar disponibilidad" as UC3
 usecase "Mostrar error: rango de horario invalido" as UC4
}
Administrador --> UC1
Administrador --> UC3
UC1 .> UC2 : <<include>>
...
```plantuml
@startuml HU02_secuencia
actor Administrador
participant "Interfaz de Staff" as UI
participant "Controlador de Barbero" as CTRL
database "Base de Datos" as DB

Administrador -> UI : Carga nombre, telefono y horarios semanales
UI -> CTRL : registrarBarbero(datos, horarios)
alt Hora fin anterior a hora inicio (en algun horario cargado)
 CTRL -> UI : error de rango de horario invalido
 UI --> Administrador : Muestra mensaje de error
else Horarios validos
 CTRL -> DB : guardarBarbero(datos)
 DB --> CTRL : idBarbero
 CTRL -> DB : guardarHorarios(idBarbero, horarios)
 DB --> CTRL : confirmacion
 CTRL -> UI : registro exitoso
 UI --> Administrador : Muestra barbero en listado de staff
end

== Edicion posterior de disponibilidad ==
Administrador -> UI : Edita horario de un barbero existente
UI -> CTRL : actualizarHorario(idBarbero, nuevoHorario)
CTRL -> DB : actualizarHorario(idBarbero, nuevoHorario)
DB --> CTRL : confirmacion
CTRL -> UI : cambios reflejados
UI --> Administrador : Disponibilidad actualizada en modulo de agendamiento
@enduml
```

### 7.3 HU-03: Disponibilidad de Horarios en Tiempo Real
```plantuml
@startuml HU03_casos_uso
left to right direction
actor Cliente
rectangle "Sistema BarberLosPeluchitos" {
 usecase "Consultar disponibilidad" as UC1
 usecase "Filtrar turnos pasados" as UC2
 usecase "Mostrar sin turnos disponibles" as UC3
 usecase "Actualizar disponibilidad en tiempo real" as UC4
}
Cliente --> UC1
UC1 .> UC2 : <<include>>
UC3 .> UC1 : <<extend>>
...
```plantuml
@startuml HU03_secuencia
actor Cliente
participant "Interfaz de Agenda" as UI
participant "Controlador de Disponibilidad" as CTRL
database "Base de Datos" as DB

Cliente -> UI : Selecciona barbero y fecha
UI -> CTRL : consultarDisponibilidad(idBarbero, fecha)
CTRL -> DB : obtenerHorarios(idBarbero, fecha)
DB --> CTRL : lista de horarios
CTRL -> CTRL : filtrarTurnosPasados(fecha, horaActual)
alt No hay turnos disponibles
 CTRL -> UI : lista vacia
 UI --> Cliente : "Sin turnos disponibles para esta fecha" + opcion siguiente dia
else Existen turnos disponibles
 CTRL -> UI : lista de turnos libres
 UI --> Cliente : Muestra horarios disponibles
end

== Actualizacion en tiempo real ==
Cliente -> UI : Intenta seleccionar un turno
UI -> CTRL : verificarDisponibilidad(idTurno)
CTRL -> DB : consultarEstadoTurno(idTurno)
DB --> CTRL : estado actual
alt Turno tomado por otro usuario
 CTRL -> UI : turno ya no disponible
 UI --> Cliente : Actualiza agenda y bloquea seleccion
else Turno sigue libre
 CTRL -> UI : turno confirmado como libre
 UI --> Cliente : Permite continuar seleccion
end
@enduml
```

### 7.4 HU-04: Agendamiento de Cita y Estados
```plantuml
@startuml HU04_estados
[*] --> EnSeleccion : Cliente elige servicio/barbero/horario
EnSeleccion --> Pendiente : Confirma reserva y turno disponible
EnSeleccion --> Rechazada : Turno tomado por otra solicitud
Rechazada --> EnSeleccion : Cliente elige otro turno
Pendiente --> Atendida : Administrador marca corte atendido
Pendiente --> Cancelada : Cliente o Admin cancela cita
Atendida --> [*]
Cancelada --> [*]
@enduml
```
...
1. **Rendimiento y Latencia:** La respuesta a consultas de disponibilidad de turnos no debe exceder los 300 ms en condiciones normales de tráfico.
2. **Disponibilidad y Concurrencia:** La WebApp debe estar disponible 24/7 con soporte para múltiples peticiones concurrentes sin generar dobles reservas gracias al aislamiento transaccional de PostgreSQL.
3. **Diseño Adaptativo (Mobile-First):** La interfaz de usuario debe ofrecer una experiencia fluida e intuitiva en pantallas de smartphones (desde 360px de ancho) y navegadores de escritorio.
4. **Seguridad y Privacidad:** Cumplimiento estricto de la Ley 164 y D.S. 1793 mediante canales cifrados HTTPS/TLS y hashing robusto de contraseñas.
